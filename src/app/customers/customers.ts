  ...
  ngOnInit() {
    this.systemService.allSuperCustomers().subscribe((res) => {
      ...
      this.customers = res.customers
        ...
        .filter((customer) => customer !== null) as Customer[]; // Filter out null values

      // Debug information
      if (this.customers.length > 0) {
        console.log('First customer object:', this.customers[0]);
        console.log('First customer ID:', this.customers[0].id);
        console.log('ID type:', typeof this.customers[0].id);
      } else {
        console.log('No valid customers found in the response');
      }
    });
  }
  ...
  applyBulkAction() {
    ...
    this.modalAction = () => {
      switch (this.bulkAction) {
        case 'enable':
          this.systemService
            .bulkUpdateStatus(this.selectedCustomers, 'active')
            .subscribe({
              next: (res) => {
              ...
              error: (error) => {
                this.showInfoModal(
                  'Error enabling customers. Please try again.'
                );
              },
            });
          break;
        case 'disable':
          this.systemService
            .bulkUpdateStatus(this.selectedCustomers, 'expired')
            .subscribe({
              next: (res) => {
              ...
              error: (error) => {
                this.showInfoModal(
                  'Error disabling customers. Please try again.'
                );
              },
            });
          break;
        case 'mark-paid':
          this.systemService
            .bulkUpdatePaymentStatus(this.selectedCustomers, 'paid')
            .subscribe({
              next: (res) => {
              ...
              error: (error) => {
                this.showInfoModal(
                  'Error marking customers as paid. Please try again.'
                );
              },
            });
          break;
        case 'mark-unpaid':
          this.systemService
            .bulkUpdatePaymentStatus(this.selectedCustomers, 'unpaid')
            .subscribe({
              next: (res) => {
              ...
              error: (error) => {
                this.showInfoModal(
                  'Error marking customers as unpaid. Please try again.'
                );
              },
            });
          break;
